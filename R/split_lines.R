...
st_split_lines <- function(input_lines, max_length, id = "ID") {
  ...
  too_long <- dplyr::mutate(too_long,
                     ...
                     fID = 1:dplyr::nrow(too_long))

  split_points <- sf::st_set_geometry(too_long, NULL)[rep(dplyr::seq_len(base::nrow(too_long)), too_long[["pieces"]]),]

  split_points <- dplyr::mutate(split_points, split_fID = dplyr::row.names(split_points)) |>
